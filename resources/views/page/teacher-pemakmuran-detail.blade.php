    ...
    <div class="teacher-project-page">
        ...
        <header class="page-header-unified center">
            <div class="header-top">
                <a href="/teacher-planner-pemakmuran/{{ $userguru->id }}" class="nav-header-back">
                ...
                <div class="note-input-section" style="width: 100%;">
                    <div class="note-section-header">
                        <h3 style="font-size: 16px; font-weight: 700; color: #1F2937; margin: 0;">
                            <i class="ph-bold ph-article" style="color: #7F56D9;"></i> Detail {{ $title }}
                        </h3>
                    ...
                    <div class="note-form-card">
                        ...
                        <div id="pemakmuran-editor" class="note-editor"
                            contenteditable="{{ auth()->id() == $userguru->id ? 'true' : 'false' }}"
                            style="min-height: 400px;"
                            data-placeholder="{{ auth()->id() == $userguru->id ? 'Tuliskan detail per bulan di sini...' : 'Belum ada data untuk bulan ini.' }}">
                        </div>

                        @if(auth()->id() == $userguru->id)
                            <div class="cal-form-actions">
                                <button id="clear-content-btn" class="btn-cal-secondary">
                                    <i class="ph-bold ph-trash"></i> Hapus
                                </button>
                                <button id="save-content-btn" class="btn-cal-primary">
        ...
        $(document).ready(function () {
            ...
            if (isOwner) {
                setInterval(function () {
                    const currentContent = $('#pemakmuran-editor').html();
                    if (currentContent !== lastSavedContent && !isSaving) {
                        saveContent(undefined, true);
                    }
                }, 60000);
            }
            ...
            $('#clear-content-btn').on('click', function () {
                if (confirm('Hapus konten bulan ini?')) {
                    $('#pemakmuran-editor').html('');
                    saveContent('');
                }
            });
            ...
            function updateUI() {
                ...
                $('#current-month-label, #selected-month-label').text(label);
            }

            function loadContent() {
                $('#pemakmuran-editor').attr('data-placeholder', 'Memuat...');
                $.ajax({
                    url: '{{ route("teacher.pemakmuran.get") }}',
                    method: 'GET',
                    data: {
                        user_id: {{ $userguru->id }},
                    ...
                    success: function (res) {
                        ...
                        $('#pemakmuran-editor').html(content).attr('data-placeholder', 'Tuliskan detail per bulan di sini...');
                        lastSavedContent = content;
                    }
                });
            }

            function saveContent(overrideContent, isAutoSave = false) {
                if (isSaving) return;

                const content = overrideContent !== undefined ? overrideContent : $('#pemakmuran-editor').html();
                const cleanContent = ($('#pemakmuran-editor').text().trim() === '' && content.indexOf('<') === -1) ? '' : content;

                // Don't save if content hasn't changed
                if (cleanContent === lastSavedContent && overrideContent === undefined) return;

                isSaving = true;
                const btn = $('#save-content-btn');
                ...
                $.ajax({
                    url: '{{ route("teacher.pemakmuran.save") }}',
                    method: 'POST',
                    data: {
                        user_id: {{ $userguru->id }},
                        year: currentDate.getFullYear(),
